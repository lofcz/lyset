...
through [`rdocx`](https://github.com/tensorbee/rdocx). Word and PDF share
one layout engine, so they look the same.
...
`rdocx` is a path dependency on the sibling checkout at `../rdocx`.
`system-fonts` is off so output is deterministic (bundled Carlito / Caladea /
Liberation).
